...
function! s:GetFullPath(linen)
    ...
    let dirp = getline(1)
    let dirm = ''
    let line = s:GetLineRemoveFlag(a:linen)
    let fname = ''
    let lvl = s:GetLvl(line)
    let lvln = a:linen
    while lvl > 0
        let fname = s:easytree_path_sep.s:GetFName(s:GetLineRemoveFlag(lvln)).fname
        let lvl -= 1
        if lvl > 0
            while s:GetLvl(s:GetLineRemoveFlag(lvln)) != lvl
                let lvln -= 1
            endwhile
        endif
    endwhile
